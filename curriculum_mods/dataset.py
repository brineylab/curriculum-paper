...
class MixedProbDataset(Dataset):
    def __init__(
        ...
        curr_prob={
            ...
            "A": -1,
            "B": 1,
        },  # if constant_prob=False, these are default values for the curr prob function
        ...
        if is_train:
            ...
            self.current_prob = (
                calc_prob(0, num_training_steps, curr_prob)
                if not constant_prob
                else prob
            )
    ...
    def __len__(self):
        if self.is_train:
            ...
            unpaired_len = (
                len(self.unpaired_data) if self.unpaired_data is not None else 0
            )
