...
def inference_heavy(model, tokenizer, seq, sep):
    ...
    with torch.no_grad():
        unmasked = tokenizer((seq + sep), return_tensors="pt").to(device)["input_ids"]

        for m in range(len(seq)):
            hmask = seq[:m] + "<mask>" + seq[m + 1 :] + sep
            i = tokenizer(hmask, return_tensors="pt").to(device)
...
def inference_light(model, tokenizer, seq, sep):
    ...
    with torch.no_grad():
        unmasked = tokenizer((sep + seq), return_tensors="pt").to(device)["input_ids"]
