...
class MixedProbCallback(TrainerCallback):
    ...
    def on_step_begin(self, args, state, control, **kwargs):
        if not self.dataset.constant_prob:
            prob = calc_prob(
                state.global_step, self.num_training_steps, self.dataset.curr_prob
            )
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        if step % state.logging_steps == 0:
            ...
            unpaired_count_tensor = torch.tensor(
                [self.dataset.unpaired_count],
                dtype=torch.int64,
                device=self.accelerator.device,
            )
            paired_count_tensor = torch.tensor(
                [self.dataset.paired_count],
                dtype=torch.int64,
                device=self.accelerator.device,
            )

            # accelerate reduce (sum)
            unpaired_count_tensor = self.accelerator.reduce(
                unpaired_count_tensor, reduction="sum"
            )
            paired_count_tensor = self.accelerator.reduce(
                paired_count_tensor, reduction="sum"
            )

            # calculate epoch fractions
            unpaired_epoch = (
                unpaired_count_tensor.item() / len(self.dataset.unpaired_data)
                if self.dataset.unpaired_data != None
                else 0
            )
            paired_epoch = (
                paired_count_tensor.item() / len(self.dataset.paired_data)
                if self.dataset.paired_data != None
                else 0
            )
    ...
    def on_evaluate(self, args, state, control, **kwargs):
        ...
        if self.eval_unpaired is not None and self.eval_paired is not None:
            ...
            eval_weighted_loss = (self.eval_unpaired * self.dataset.current_prob) + (
